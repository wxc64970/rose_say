...
class RssubscribeController extends GetxController {
  RssubscribeController();

  final state = RssubscribeState();

  // 响应式状态变量
  final RxList<RSSkModel> skuList = <RSSkModel>[].obs;
  final Rx<RSSkModel?> selectedProduct = Rx<RSSkModel?>(null);
  ...
  Future<void> _loadSubscriptionData() async {
    try {
      ...
      skuList.value = RSPayUtils().subscriptionList;
  ...
  void _updateContentText() {
    if (RS.storage.isRSB) {
      ...
      contentText.value = RSTextData.vipGet2(gems.toString());
    } else {
      contentText.value = RSTextData.vipGet;
    }
  }

  /// 记录页面事件
  void _logPageEvent() {
    RSlogEvent(RS.storage.isRSB ? 't_vipb' : 't_vipa');
  }
  ...
  /// 选择商品
  void selectProduct(RSSkModel product) {
  ...
  void purchaseSelectedProduct() {
    ...
    RSlogEvent(RS.storage.isRSB ? 'c_vipb_subs' : 'c_vipa_subs');
    RSPayUtils().buy(product, vipFrom: vipFrom);
  }
  ...
  String get currentUnit {
    ...
    if (skuType == 2) return RSTextData.month;
    if (skuType == 3) return RSTextData.year;
    return '';
  }
  ...
  String get subscriptionDescription {
    ...
    if (skuType == 4) {
      return RSTextData.vipPriceLtDesc(price);
    } else {
      return RSTextData.subscriptionInfo(price, unit);
    }
  }
