...
class RscallguideController extends GetxController
    with WidgetsBindingObserver, RouteAware {
  RscallguideController();
  ...
  Future<void> _downloadAndInitVideo() async {
    try {
      ...
      final path = await FileDownloadService.instance.downloadFile(
        url,
  ...
  void _videoListener() {
    ...
    if (timeRemaining <= const Duration(milliseconds: 500)) {
      ...
      if (RS.login.vipStatus.value && !_hasCalledPhoneAccept) {
  ...
  void phoneAccept() async {
    ...
    if (vip) {
      if (RS.login.gemBalance.value < ConsumeFrom.call.gems) {
        Get.toNamed(RSRouteNames.gems, arguments: ConsumeFrom.call);
        return;
      }
  ...
  void pushVip() {
    RSlogEvent('c_unlock_videocall');
    Get.toNamed(RSRouteNames.vip, arguments: VipFrom.call);
  }
