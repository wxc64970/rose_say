...
class RsmaskController extends GetxController {
  RsmaskController();

  final state = RsmaskState();
  ...
  void handleChangeMask() async {
    if (needConfirmChange) {
      DialogWidget.alert(
        message: RSTextData.maskAlreadyLoaded,
  ...
  Future<void> onLoad() async {
    ...
    refreshController.finishLoad(
      state.hasMore.value ? IndicatorResult.none : IndicatorResult.noMore,
    );
  }
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final records = await Api.getMaskList(
      ...
      // 自动选择当前会话的 mask
      if (state.selectedMask.value == null &&
          state.maskList.isNotEmpty &&
          msgCtr.state.session.profileId != null) {
        state.selectedMask.value = state.maskList.firstWhereOrNull(
          (element) => element.id == msgCtr.state.session.profileId,
        );
      }
      ...
      state.emptyType.value = state.maskList.isEmpty
          ? EmptyType.noNetwork
          : null;
      if (state.currentPage.value > 1) {
        state.currentPage.value -= 1;
  ...
  /// 选择 mask
  void selectMask(RSMaskModel mask) {
    state.selectedMask.value = mask;
  }

  /// 推送编辑页面
  Future<void> pushEditPage({RSMaskModel? mask}) async {
    await Get.toNamed(RSRouteNames.editMask, arguments: mask);
    onRefresh();
  }
