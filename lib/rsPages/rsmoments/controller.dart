...
class RsmomentsController extends GetxController {
  RsmomentsController();

  final EasyRefreshController cjRefreshController = EasyRefreshController(
  ...
  EmptyType? type = EmptyType.RSLoading;
  ...
  Future<void> onLoad() async {
    await _fetchData();
    cjRefreshController.finishLoad(
      isNoMoreData ? IndicatorResult.noMore : IndicatorResult.none,
    );
  }

  Future<List<RSPost>?> _fetchData() async {
    try {
      final records = await Api.momensListPage(page: page, size: size) ?? [];
      type = records.isEmpty
          ? (RSNetworkMonitorService.to.isConnected == false
                ? EmptyType.noNetwork
                : EmptyType.noData)
          : type;
      loading = records.isEmpty
          ? (RSNetworkMonitorService.to.isConnected == false ? true : true)
          : loading;
  ...
  void onItemClick(RSPost data) async {
    ...
    try {
      ...
      final (role, session) = await (
        Api.loadRoleById(chaterId),
        Api.addSession(chaterId),
      ).wait;
  ...
  void onPlay(RSPost data) {
    var isVideo = data.cover != null && data.duration != null;
    var imgUrl = isVideo ? data.cover : data.media;

    if (isVideo) {
      if (data.media != null) {
        Get.toNamed(RSRouteNames.videoPreview, arguments: data.media!);
      } else {
        SmartDialog.showToast('No video');
      }
    } else {
      Get.toNamed(RSRouteNames.imagePreview, arguments: imgUrl ?? '');
    }
  }
