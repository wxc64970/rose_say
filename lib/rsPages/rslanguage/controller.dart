...
class RslanguageController extends GetxController {
  RslanguageController();

  final state = RslanguageState();

  List<RSAzListContactModel> contactList = [];
  ...
  var choosedName = ''.obs;
  RSLang? selectedLang; // 保存选中的语言对象
  Rx<EmptyType?> emptyType = Rx<EmptyType?>(null);

  Future<void> generateContactData() async {
    try {
      emptyType.value = EmptyType.RSLoading;
      // 从 IConfig 获取语言数据（内部已处理缓存逻辑）
      await RS.login.loadAppLangs();
      final appLangs = RS.login.appLangs;
      ...
      debugPrint('Error RSLoading language data: $e');
  ...
  void _buildContactListFromData(Map<String, dynamic> appLangs) {
    ...
    appLangs.forEach((key, value) {
      if (value is List) {
        ...
        if (names.isNotEmpty) {
          contactList.add(
            RSAzListContactModel(section: key, names: names, langs: langs),
          );
        }
      }
    });
  ...
  void onSaveButtonTapped() async {
    if (selectedLang != null) {
      ...
      if (isOK) {
        RS.login.sessionLang.value = selectedLang;
      ...
      // 可以显示提示信息
      RSToast.show('Please select a language');
    }
  }
  ...
  void _setDefaultSelectedLanguage() {
    try {
      ...
      final matchedLang = RS.login.matchUserLang();
