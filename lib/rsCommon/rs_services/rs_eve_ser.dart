...
void RSlogEvent(String name, {Map<String, Object>? parameters}) {
  try {
    log.d('[RSlogEvent]: $name, $parameters');
    if (RSThirdPartyService.isFirebaseInitialized) {
      FirebaseAnalytics.instance.logEvent(name: name, parameters: parameters);
    }
    RSAppLogEvent().logCustomEvent(name: name, params: parameters ?? {});
  } catch (e) {
...
/// 日志数据库服务
class RSLogEventDBService {
  static final RSLogEventDBService _instance = RSLogEventDBService._internal();
  factory RSLogEventDBService() => _instance;
  RSLogEventDBService._internal();

  static Box<RSEventData>? _box;
  static const String boxName = 'events_logs';
  static int _sequenceCounter = 0;
  static int _lastTimestamp = 0;

  Future<Box<RSEventData>> get box async {
    if (_box != null) return _box!;
    _box = await _initBox();
    return _box!;
  }

  Future<Box<RSEventData>> _initBox() async {
    ...
    return await Hive.openBox<RSEventData>(boxName);
  }
  ...
  Future<void> insertLog(RSEventData log) async {
    final box = await this.box;
    return await box.put(log.id, log);
  }

  Future<List<RSEventData>> getUnuploadedLogs({int limit = 10}) async {
    final box = await this.box;
  ...
  Future<List<RSEventData>> getFailedLogs({int limit = 10}) async {
    final box = await this.box;
  ...
  Future<void> markLogsAsSuccess(List<RSEventData> logs) async {
    final box = await this.box;
    final now = DateTime.now().millisecondsSinceEpoch;
    try {
      for (final log in logs) {
        final updatedLog = RSEventData(
          id: log.id,
...
class RSAppLogEvent {
  static final RSAppLogEvent _instance = RSAppLogEvent._internal();

  factory RSAppLogEvent() => _instance;

  RSAppLogEvent._internal() {
    _startTimersAsync();
  }

  final _adLogService = RSLogEventDBService();
  Timer? _uploadTimer;
  Timer? _retryTimer;
  ...
  Future<Map<String, dynamic>?> _getCommonParams() async {
    final uniqueTimestamp = RSLogEventDBService.generateUniqueTimestamp();

    try {
      final deviceId = await RS.storage.getDeviceId(isOrigin: true);
      final deviceModel = await RSInfoUtils.getDeviceModel();
      final manufacturer = await RSInfoUtils.getDeviceManufacturer();
  ...
  Future<void> logInstallEvent() async {
    try {
      ...
      final build = await RSInfoUtils.buildNumber();
      final isLimitAdTrackingEnabled =
          await RSInfoUtils.isLimitAdTrackingEnabled();
      final agent = RSInfoUtils.userAgent();
      ...
      log.e('[ad]log RSlogEvent error: $e');
    }
  }
  ...
  Future<void> _uploadPendingLogs() async {
    try {
      ...
      // 过滤掉正在上传中的日志，避免重复上传
      final logs = allLogs
          .where((log) => !_uploadingLogIds.contains(log.id))
          .toList();
            ...
            try {
              await _adLogService
                  ...
                  .timeout(
                    ...
                    onTimeout: () {
                      log.w(
                        '[ad]log markLogsAsSuccess timeout, retry: $retryCount',
                      );
                      throw TimeoutException(
  ...
  void dispose() {
    ...
    _uploadTimer = null;
    _retryTimer = null;
    log.d('[ad]log RSAppLogEvent disposed');
  }
}
