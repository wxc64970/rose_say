...
class RSLoginService extends GetxService {
  ...
  RSUsersModel? _currentUser;

  RSUsersModel? get currentUser => _currentUser;

  /// 各种价格配置
  RSPricesModel? configPrice;

  /// 支持的语言列表
  Map<String, dynamic>? appLangs;

  final sessionLang = Rxn<RSLang>();

  Future<void> performRegister() async {
    try {
      final userCached = RS.storage.user;
      if (userCached != null) {
  ...
  void saveUserInfo(RSUsersModel? user) async {
    if (user == null) {
  ...
  void _updateAppLangsInBackground() {
    Api.getAppLangs()
        .then((data) {
          if (data != null) {
            appLangs = data;
            RS.storage.setAppLangs(json.encode(data));
  ...
  Future<void> loadAppLangs() async {
    try {
      // 先从缓存获取数据
      final cachedData = RS.storage.appLangs;
      if (cachedData != null) {
  ...
  RSLang matchUserLang() {
    // 获取所有可用语言列表
    final allLangs = _getAllAvailableLangs();
    if (allLangs.isEmpty) {
      return RSLang(label: 'English', value: 'en');
    }
  ...
  /// 匹配设备语言
  RSLang _matchDeviceLanguage(List<RSLang> allLangs) {
  ...
  /// 匹配用户设置的语言
  RSLang _matchUserLanguage(String userLang, List<RSLang> allLangs) {
    // 1. 精确匹配
    for (var lang in allLangs) {
  ...
  /// 查找默认语言 (优先英语)
  RSLang _findDefaultLanguage(List<RSLang> allLangs) {
    // 优先查找英语
    for (var lang in allLangs) {
      ...
      if (value is List) {
        for (var item in value) {
          if (item is Map<String, dynamic>) {
            try {
              final lang = RSLang.fromJson(item);
