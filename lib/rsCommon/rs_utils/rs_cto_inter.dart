...
class RSCryptoInterceptor extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    try {
      // 1. 加密 URL（路径和查询参数）
      final encryptedUrl = RSCryptoUtil.encryptUrl(
        originalUrl: options.uri.toString(),
        prefix: RSAppConstants.apiPrefix,
      );
      ...
      if (options.contentType == 'multipart/form-data') {
        ...
        options.data = RSCryptoUtil.encryptParams(options.data);
  ...
  void onResponse(Response response, ResponseInterceptorHandler handler) async {
    ...
    try {
      final decryptedData = RSCryptoUtil.decrypt(response.data); // 实际需替换为对称解密
