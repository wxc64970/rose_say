...
class RSPayUtils {
  // 单例模式
  static final RSPayUtils _instance = RSPayUtils._internal();
  factory RSPayUtils() => _instance;
  RSPayUtils._internal() {
    _initIAP();
  }
  ...
  List<RSSkModel> allList = [];
  List<RSSkModel> consumableList = [];
  List<RSSkModel> subscriptionList = [];

  VipFrom? _vipFrom;
  ConsumeFrom? _consFrom;
  RSOrderModel? _orderData;

  bool _isUserBuy = false;
  RSSkModel? _currentSkuData;
  ...
  Future<void> buy(
    ...
    try {
      ...
      if (productDetails == null) {
        ...
        RSToast.show('So sorry, ProductDetails is null.');
        return;
      }
      ...
      if (orderNo == null || orderNo.isEmpty) {
        RSLoading.close();
        RSToast.show(RSTextData.createOrderError);
        return;
      }
  ...
  Future<void> _createOrder(ProductDetails productDetails) async {
    ...
    if (Platform.isIOS) {
      try {
        ...
        // RSToast.show('${L10nHelper.current.createOrderError} $e');
        RSToast.show('So sorry, create order error. $e');
        rethrow;
      }
    }
  ...
  Future<bool> _isAvailable() async {
    ...
    if (!isAvailable) {
      await RSLoading.show();
      // RSToast.show(L10nHelper.current.iapNotSupport);
      RSToast.show('So sorry, IAP not support.');
    }
    return isAvailable;
  }
  ...
  void _reportPurchase(PurchaseDetails purchaseDetails) {
    ...
    if (_consumableIds.contains(id)) {
      ...
      if (_consFrom == ConsumeFrom.generateimage ||
          _consFrom == ConsumeFrom.home ||
          _consFrom == ConsumeFrom.profile ||
          _consFrom == ConsumeFrom.album ||
          _consFrom == ConsumeFrom.chat) {
        return;
      }
      Get.back();
    } else {
  ...
  void _showRechargeSuccess(String productID) {
    RSlogEvent('t_suc_gems');

    final number = _currentSkuData?.number ?? 0;
    DialogWidget.showRechargeSuccess(number);
  }

  void _handleError(IAPError error) {
    RSLoading.close();
    log.e('[iap] 错误: ${error.message}');
    RSToast.show(error.message);
  }
